    ...
    /*json数据如下
        {
            ...
            "token": "d26efa30b6a01cdaa16758bfd02ad96b",
            "user": "liult"
        }
    */
...
int share_file(char *user, char *md5, char *filename)
{
    ...
    */
    ...
    // 文件标示，md5+文件名。
    // 因为md5是唯一的，且位数固定（32位），所以可以很方便的获取文件名
...
int del_file(char *user, char *md5, char *filename)
{
    /*
        1、先判断此文件是否已经分享
        2、判断集合有没有这个文件，如果有，说明别人已经分享此文件(redis操作)
        3、如果集合没有此元素，可能因为redis中没有记录，再从mysql中查询，如果mysql也没有，说明真没有(mysql操作)
        4、如果mysql有记录，而redis没有记录，那么分享文件处理只需要处理mysql (mysql操作)
        5、如果redis有记录，mysql和redis都需要处理，删除相关记录
    */
    int ret = 0;
    char sql_cmd[SQL_MAX_LEN] = {0};
    MYSQL *conn = NULL;
