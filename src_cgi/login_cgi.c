...
int check_user_pwd( char *username, char *pwd )
{
    ...
    char mysql_db[256] = {0};
    get_mysql_info(mysql_user, mysql_pwd,  mysql_db);   // 去配置文件读取数据
    ...
    // 返回值：0成功并保存结果集，1没有结果集，2有结果集但是没有保存，-1失败
...
int main()
{
    ...
    while (FCGI_Accept() >= 0)
    {
        char *contentLength = getenv("CONTENT_LENGTH");     // 获取参数
        int len;
        char token[128] = {0};
        ...
        else
        {
            ...
            char buf[4*1024] = {0};
            int ret = 0;
            ret = fread(buf, 1, len, stdin);    // 从标准输入（web服务器）（nginx）读取内容
            if(ret == 0)
            {
            ...
            char username[512] = {0};
            char pwd[512] = {0};
            get_login_info(buf, username, pwd);     // 使用CJSON
